...
## Two interviewers respond differently to the same hiring judgment
...
<div class="criterion"><strong>The right answer depends on the claim.</strong> If the study asks how evaluators construct cultural fit, A currently gives better evidence. If it requires a standardized measure of fit, neither exchange is an adequate instrument.</div>
...
<table class="tbl"><tr><th>Same before the answer</th><th>Different after the answer</th></tr><tr><td>Research question<br>Fixed opening question<br>Instructions about probing</td><td><strong>A:</strong> asks what “in common” meant<br><strong>B:</strong> supplies “firm culture” as the interpretation</td></tr></table>
...
## Rivera uses interviews to explain how employers judge “fit”

<div class="cols6040"><img class="paper" src="images/rivera_title_crop.png" alt="Title and abstract of Rivera's Hiring as Cultural Matching"><div><p>The study draws on <strong>120 interviews</strong> with evaluators in elite professional-service firms and participant observation of one hiring committee.</p><p>Rivera argues that hiring is not only skills sorting. Evaluators also match candidates to themselves and to an image of the firm.</p><p>This gives us a substantive question for the rest of the lecture: what must an interviewer ask to show how a judgment of fit was made?</p></div></div>

<div class="citation">Rivera, L. A. (2012). “Hiring as Cultural Matching: The Case of Elite Professional Service Firms.” <em>American Sociological Review</em> 77(6):999–1022. https://doi.org/10.1177/0003122412463213.</div>

---

## Interviews and observation contribute different evidence

<div class="cols6040"><img class="paper" src="images/rivera_methods_crop.png" alt="Rivera's methods sections on interviews and participant observation"><div><p><strong>Interviews:</strong> evaluators explained criteria, assessed mock résumés and discussed recently interviewed candidates.</p><p><strong>Observation:</strong> committee meetings showed how candidate evaluations were produced and contested in real time.</p><p>The two sources do not do the same job. The interview reveals evaluators' accounts; observation shows parts of the decision process outside those accounts.</p></div></div>
...
opening_question = (
    "Tell me about a recent candidate who seemed like a good fit."
)
first_answer = (
    "The conversation flowed. We had a surprising amount in common."
)
turns = [
    {"role": "interviewer", "text": opening_question},
    {"role": "participant", "text": first_answer},
]

messages = [
    {"role": "system", "content": INTERVIEW_PROTOCOL},
    {"role": "user", "content": json.dumps(turns, indent=2)},
]
```

<div class="io-strip"><div><strong>Input</strong><br>Two strings become two turn dictionaries inside the ordered <code>turns</code> list.</div><div><strong>Output</strong><br><code>json.dumps(...)</code> converts that list into readable text inside <code>messages</code>.</div></div>
...
## Save the generated probe with the answer that prompted it

```python
probe_record = {
    "participant_answer": first_answer,
    "generated_probe": next_probe,
    "source": "adaptive_probe",
    "model": INTERVIEW_MODEL,
}
...
<div class="purpose"><strong>Input:</strong> the participant's answer, the generated question and the model name. <strong>Output:</strong> one dictionary preserving the connection between them.</div>

---

## A list can preserve each interview turn in order

```python
conversation = [
    {"role": "interviewer", "text": opening_question},
    {"role": "participant", "text": first_answer},
    {"role": "interviewer", "text": next_probe},
]

print(conversation[2]["text"])
```

<div class="cols"><div><strong><code>conversation</code></strong><br>A list. Its order records what happened first, second and third.</div><div><strong><code>conversation[2]</code></strong><br>The third dictionary. Python starts counting at zero.</div></div>

---

## Now add the participant's next answer

```python
second_answer = (
    "We had both worked on the college newspaper, "
    "so the conversation became much more informal."
)
...
<div class="purpose"><code>.append(...)</code> changes the list by adding one new item at the end. That expanded conversation becomes the input to the next model call.</div>
...
Generate one adaptive probe, add one participant answer, and explain orally:
