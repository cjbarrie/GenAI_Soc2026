# Instructor notes — Session 1: Foundations, Python data, and course map

## Intended endpoint

Students should leave able to defend this bounded claim: **An LLM becomes a research technology only when its input, transformation, output, and validation record are visible.** They should also explain `build_research_record` line by line without live AI assistance.
...
- The three arguments are existing Python objects: two strings and one dictionary.
- `settings.copy()` makes a new dictionary so later edits do not silently alter the saved record.
- The returned dictionary nests related fields under names a researcher can inspect.
- `None` means validation has not happened; it does not mean the output is valid.
