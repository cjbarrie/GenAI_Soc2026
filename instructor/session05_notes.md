# Instructor notes — Session 5: Treatment and stimulus generation

## Intended endpoint

Students should leave able to defend this bounded claim: **Generating many stimuli increases design capacity only if intended and nuisance variation are inspected separately.** They should also explain `construct_treatments` line by line without live AI assistance.
...
- The outer loop chooses one frame; the inner loop pairs it with every tone.
- The f-string creates a stable condition name used to retrieve its word count.
- Each dictionary is one treatment record, not the treatment text itself.
- The conditional applies the rule declared before looking at outcomes.
