# Instructor notes — Session 13: Auditing LLMs as objects of study

## Intended endpoint

Students should leave able to defend this bounded claim: **An output difference is an audit finding; explaining its social cause requires a stronger design.** They should also explain `factorial_audit` line by line without live AI assistance.
...
- Three nested loops construct the Cartesian product one factor at a time.
- A tuple is the exact key for one model-language-frame condition.
- The membership check distinguishes an unobserved condition from a response value of zero.
- The outputs describe what was observed and what is missing; neither output supplies a causal explanation.
